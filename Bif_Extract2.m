function [dataname,alpha_final,linfit,expofit,lin_r0,expo_r0] = Bif_Extract2(CLFILE,BVFILE)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Author: M. J. Colebank
...
%%  Load
nodesname = strcat(CLFILE(1:end-6),'Nodes.mat');
dataname = strcat(CLFILE(1:end-6),'Data.mat');
tapername = strcat(CLFILE(1:end-6),'Taper.mat');
...
cl = [r x y z];
%% Do a similar thing for all the bifurcation information
fid = fopen(BVFILE);
C = textscan(fid, '%s');
fclose(fid);
header = char(C{1}(1));
...
headernames{headernameID} = header(startID:i);
%Now find the stuff you want
bvread = csvread(BVFILE,1);
for j=1:headernameID
    if strcmp(headernames{j},groupIDnamequotes) || strcmp(headernames{j},groupIDname)
        bif_IDs = bvread(:,j);
    elseif strcmp(headernames{j},xnamequotes) || strcmp(headernames{j},xname)
        bvx = bvread(:,j);
    elseif strcmp(headernames{j},ynamequotes) || strcmp(headernames{j},yname)
...
scalingfactor = 0;%0.86;%input(prompt);
% if flagscale > 0
%     scalingfactor = flagscale;
% else
%     scalingfactor = 0.86;
% end
%% Test to see if trifurcations or bifurcations with only 2 vector
%  points exist in the file (looking at group ID in .vtp file)
bvsize = size(bvcoord,1);
false_bif = [];
false_bif_parent = [];
trif = [];
trif_parent = [];
...
n_times = [];
ntime_inc = 1;
for b = 1:length(bif_IDs) %A loop to decide how many
    check = bif_IDs(b);   % daughters are with each group ID
    numdup = 0;
    for j=1:length(bif_IDs)
        if check == bif_IDs(j)
            numdup = numdup+1;
        end
    end
    if numdup < 3
        false_bif = [false_bif b]; %no bifurcation
    elseif numdup == 4
        trif = [trif b]; %a trifurcation
    elseif numdup > 4
        n_times(ntime_inc)    = numdup;
        ntime_inc      = ntime_inc+1;
        nfurcation = [nfurcation b];
    else
        bif  = [bif b];
    end
end
%% Need to index where each set of bifurcations begin
if ~isempty(false_bif)
    false_bif_parent = false_bif(1:2:end);
end
if ~isempty(bif)
    bif_parent = bif(1:3:end);
end
if ~isempty(trif)
    trif_parent = trif(1:4:end);
end
if ~isempty(nfurcation)
    start = 1;
    i=1;
    while i < length(n_times) %Finds the parent of each n-furcation
        nfurcation_parent(start) = nfurcation(i);
        i = i+n_times(i);
        start = start + 1;
    end
end
...
diff_nfur = diff(nfurcation);
diff_nfur(end+1) = 10^6;    %If we assume nfurcations don't happen consecutively
where_nfur = find(diff_nfur > 1);
start = 1;
for i=1:length(where_nfur)
    nfur_number(i) = sum(diff_nfur(start:where_nfur(i)-1));
    start = where_nfur(i)+1;
end
num_bifs = length(bif)./3 + length(false_bif)./2 + length(trif)./4 + length(nfur_number);
arraylength = length(x);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Algorithm 1: New function for finding unique points
[nodes,arcs,adjust] = find_nodes3(cl); %Adjust can be used to set the root vessel to have z=0 at the inlet
load('Better_Lung_Nodes.mat');
bvcoord(:,3) = bvcoord(:,3) - adjust(3);
vessel_keep = arcs;
...
for i=1:length(nodes)-1
    if i<=length(vessel_keep)
        ...
        plot3(vessel_keep{i}(:,1),vessel_keep{i}(:,2),(-1).*vessel_keep{i}(:,3),'k');
    end
    plot3(nodes(i+1,1),nodes(i+1,2),(-1).*nodes(i+1,3),'ro','MarkerSize',10,'MarkerFace','r');
end
plot3(nodes(1,1),nodes(1,2),(-1).*nodes(1,3),'ro','MarkerSize',10,'MarkerFace','r');
...
charID = charID+1;
newlengthflag = 1;
[vessel_organized, new_vessels, connectivity, VESSEL_NAME, VESSEL_INDEX, CONN_INDEX, charID, bvignore, newlengthflag] =  ...
    find_network_cl(start_vessel,vessel_keep, vessel_organized, connectivity, VESSEL_NAME, VESSEL_INDEX, ...
    CONN_INDEX, charID, bvignore, bvcoord,bif_parent, trif_parent,false_bif_parent, ...
    nfurcation_parent, nfur_number, vessel_starts, newlengthflag);
...
toc
% save(dataname,'connectivity','vessel_details','bvcoord','centroid');
...
legend(h,{'Lin fitted','Expo fitted'})
hold off;
% save(tapername,'vessel_details','leftbranch','rightbranch','left_cum_length','right_cum_length', ...
%     'left_start','left_end','left_taper_linear','right_start','right_end','right_taper_linear')
