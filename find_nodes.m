...
if flag == 0
   ...
   zroot      = linspace(0.99*new_root_z,new_root_z*1.2,10)';
...
%     %% A manual check for the network (24,3)
%     if any(intersect_nodes(:,3) == 0.505792312622100e+02)
%        tempID = find(intersect_nodes(:,3) == 0.505792312622100e+02);
%        intersect_nodes(tempID,:) = [-294.2953491211           -258.7046508789            50.57873153687            8.580218529996];
%     end
    %%
    len_intersect = size(intersect_nodes,1);
    temp_nodes(end+1:end+len_intersect,:) = intersect_nodes;
    %%%%Here is a plotting routine if things go wrong
%                 figure(200); clf; hold on;
%                 plot3(uni_mat(:,1),uni_mat(:,2),uni_mat(:,3),'yo');
%                 plot3(cl_curr(:,1),cl_curr(:,2),cl_curr(:,3),'ro');
%                 plot3(intersect_nodes(:,1),intersect_nodes(:,2),intersect_nodes(:,3),'ks','MarkerSize',10,'MarkerFace','k');
    ...
    for t=1:len_curr
        if any(cl_curr(t,:) == intersect_nodes)
            if ~isempty(end_nodes)
                ...
                %% use for just xyz comparison
%                 dist_start = abs(sum(ones(size(start_nodes(:,1:3),1),3).*temp_start(1:3) - start_nodes(:,1:3),2));
%                 dist_end   = abs(sum(ones(size(end_nodes(:,1:3),1),3).*temp_end(1:3) - end_nodes(:,1:3),2));
%                 r_comp     = temp_end(4) + end_nodes(:,4);
%                 [who,where] = sort(dist_start);
%                 r_comp_start = r_comp(where);
%                 [who,where] = sort(dist_end);
%                 r_comp_end = r_comp(where);
                %%
