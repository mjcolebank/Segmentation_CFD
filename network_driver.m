...
%         hold off;
        ...
        for iter = 1:0
            ...
            tic
            unix(sprintf( strcat('./sor06',sprintf(' %f %f %g ', num_vessels, num_terminal, f3(iter)) ) ) );
            toc
...
% pressures{save_id,1} = pressure1; pressures{save_id,2} = pressure2; pressures{save_id,3} = pressure3;
% flows{save_id,1} = flow1; flows{save_id,2} = flow2; flows{save_id,3} = flow3;
% areas{save_id,1} = area1; areas{save_id,2} = area2; areas{save_id,3} = area3;
% wavespeeds{save_id,1} = wavespeed1; wavespeeds{save_id,2} = wavespeed2; wavespeeds{save_id,3} = wavespeed3;
% 
% BC_save{end+1} = BC_matrix;
% RT_CT{end+1}   = [Rtotal CT];
% save_id = save_id + 1;
end
% save('Network25_BCs_8generations.mat','BC_save');
% save('Network25_RTCT_8generations.mat','RT_CT');
% save('Network25_Simulations_8generations.mat','pressures','flows','areas','wavespeeds','BC_matrix')
